...
The README stays the working guide — problem, setup, workflow, commands. The deep-dive
reference lives in `docs/`:

| File | What it covers |
|---|---|
| [`docs/PLATFORMS.md`](docs/PLATFORMS.md) | macOS / Linux / Windows support, the four routes off Windows, the line this project will not cross |
...
| [`docs/STATUTE_REGISTRY.md`](docs/STATUTE_REGISTRY.md) | Where every statutory figure comes from, and the AY 2027-28 change of statute |
| [`docs/VERIFICATION.md`](docs/VERIFICATION.md) | The eleven test suites, the live Excel round-trips, and the supply chain |
...
[Stock splits](docs/COMPUTATION.md#stock-splits-the-build-stops-rather-than-guess) is about.
...
it is the only command that needs Windows, and [Platform guide](docs/PLATFORMS.md) covers
...
Stage 1 entirely and fill `transactions.csv` by hand — the [data dictionary](docs/DATA_DICTIONARY.md) defines
...
- an **Indian security**, which Schedule FA cannot disclose at all — see
  [Indian securities are refused](docs/COMPUTATION.md#indian-securities-are-refused). `build` and `threshold`
...
> see the [platform guide](docs/PLATFORMS.md), which also covers the department's macOS utility.
...
6. **reads every cell back and verifies it** — see [Verification](docs/VERIFICATION.md);
...
[Indian securities are refused](docs/COMPUTATION.md#indian-securities-are-refused) before reaching for it.
